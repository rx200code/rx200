...
const dpr = 2;// / 2;//(window.devicePixelRatio || 1); // Коэффициент плотности пикселей
...
view.onwheel = e => {// ПРОБНЫЙ ВАРИАНТ? для тестов
	...
	// разница между курсором и центром в тайлах
	let t_x_1 = (e.offsetX - viev_center_x) / tile_scale;
	let t_y_1 = (e.offsetY - viev_center_y) / tile_scale;
	
	// положение курсора в системе координат тайлов.
	...
	zoom -= Math.sign(e.deltaY) * .2;//Для совместимости браузеров, от дельты берем только знак.
	...
	deg_WM = half_length_WM / 180;// Один градус долготы Веб меркатора в тайлах.
	max_xy = 1 << z;
	...
	t_x_1 = (e.offsetX - viev_center_x) / tile_scale;
	t_y_1 = (e.offsetY - viev_center_y) / tile_scale;
	...
	ctx_buf.setTransform(
		scale_cos_a,  // a: Масштаб по X * cos
		scale_sin_a,  // b: Масштаб по X * sin (наклон)
		-scale_sin_a, // c: -Масштаб по Y * sin (наклон)
		scale_cos_a,  // d: Масштаб по Y * cos
		viev_center_x - (viev_center_x * scale_cos_a - viev_center_y * scale_sin_a), // e: расчет горизонтального смещения
		viev_center_y - (viev_center_x * scale_sin_a + viev_center_y * scale_cos_a)  // f: расчет вертикального смещения
	);
...
const initView = elm => {
	...
	const observer = new ResizeObserver(entries => {
		...
		//ctx.fillRect(0, 0, viev_center_x * 2, viev_center_y * 2);
...
nava.setAngleDeg = (a) => {
	...
	ctx_buf.setTransform(
		...
		scale_cos_a,  // d: Масштаб по Y * cos
		viev_center_x - (viev_center_x * scale_cos_a - viev_center_y * scale_sin_a), // e: расчет горизонтального смещения
		viev_center_y - (viev_center_x * scale_sin_a + viev_center_y * view_cos_a * scale)  // f: расчет вертикального смещения
	);
