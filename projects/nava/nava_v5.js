...
//const delimiter_zoom = .75;// .25
let z = (zoom + .25) | 0; 
let z_scale = 2 ** (zoom - z);
...
let _view_width_tile;
let _view_height_tile;
let _viev_center_x_tile;
let _viev_center_y_tile;
let _canvas_center_xy;
...
view.onwheel = e => {// ПРОБНЫЙ ВАРИАНТ? для тестов
	...
	//let t_x = (e.offsetX - viev_center_x) / tile_size * dpr / z_scale + center_x;
	//let t_y = (e.offsetY - viev_center_y) / tile_size * dpr / z_scale + center_y;
	
	let t_x_1 = (e.offsetX - viev_center_x) / tile_size * dpr / z_scale;
	let t_y_1 = (e.offsetY - viev_center_y) / tile_size * dpr / z_scale;
	
	let t_x = t_x_1 * view_cos_a - t_y_1 * view_sin_a + center_x;
	let t_y = t_x_1 * view_sin_a + t_y_1 * view_cos_a + center_y;
	
	//out(t_x, t_y);
	
	
	zoom -= Math.sign(e.deltaY) * .5;//Для совместимости браузеров, от дельты берем только знак.
	...
	view_width_tile = _view_width_tile / z_scale;
	view_height_tile = _view_height_tile / z_scale;
	viev_center_x_tile = _viev_center_x_tile / z_scale;
	viev_center_y_tile = _viev_center_y_tile / z_scale;
	_canvas_center_xy = canvas_center_xy * z_scale;
	let t_offset = canvas_center_xy - _canvas_center_xy;
	
	
	//let temp_x = center_x / half_length_WM;
	//let temp_y = (half_length_WM - center_y) / radius_WM;
	
	t_x = t_x / half_length_WM;
	t_y = (half_length_WM - t_y) / radius_WM;
	
	half_length_WM = 2 ** (z - 1);// половина длины Веб меркатора в тайлах(tile = 1), Math.log2(tile) = 0;
	...
	t_x_1 = (e.offsetX - viev_center_x) / tile_size * dpr / z_scale;
	t_y_1 = (e.offsetY - viev_center_y) / tile_size * dpr / z_scale;
	
	center_x = t_x - (t_x_1 * view_cos_a - t_y_1 * view_sin_a);
	center_y = t_y - (t_x_1 * view_sin_a + t_y_1 * view_cos_a);
	
	//center_x = t_x - (e.offsetX - viev_center_x) / tile_size * dpr / z_scale;
	//center_y = t_y - (e.offsetY - viev_center_y) / tile_size * dpr / z_scale;
	
	
	
	//ctx.scale(z_scale, z_scale);
	if(view_a === 0)ctx.setTransform(z_scale, 0, 0, z_scale, t_offset, t_offset);
	else ctx_buf.setTransform(z_scale, 0, 0, z_scale, t_offset, t_offset);
	...
	z -= Math.sign(e.deltaY);//Для совместимости браузеров, от дельты берем только знак.
	
	if(z < z_min)z = z_min;
	else if(z > z_max)z = z_max;
	
	//half_length_WM = 1 << (z - 1);// половина длины Веб меркатора в тайлах(tile = 1), Math.log2(tile) = 0;
	half_length_WM = 2 ** (z - 1);// половина длины Веб меркатора в тайлах(tile = 1), Math.log2(tile) = 0;
	radius_WM = half_length_WM / rad_180;// Радиус веб меркатора, если считать длину в количестве тайлов на сторону.
	max_xy = 1 << z;
...
const initView = elm => {
	...
	const observer = new ResizeObserver(entries => {
		...
		_view_width_tile = view_width / tile_size * dpr;
		_view_height_tile = view_height / tile_size * dpr;
		_viev_center_x_tile = viev_center_x / tile_size * dpr;
		_viev_center_y_tile = viev_center_y / tile_size * dpr;
		
		
		canvas_side = Math.ceil((view_width ** 2 + view_height ** 2) ** .5);
		canvas_center_xy = canvas_side / 2;
		
		canvas.width = canvas.height = canvas_buf.width = canvas_buf.height = canvas_side;// * dpr;
		//canvas.style.width = canvas.style.height = canvas_buf.style.width = canvas_buf.style.height = canvas_side + "px";
		...
		let t_offset = 0;//canvas_center_xy - _canvas_center_xy;
		let t_z_scale = z_scale / dpr;
		
		if(view_a === 0)ctx.setTransform(t_z_scale, 0, 0, t_z_scale, t_offset, t_offset);
		else ctx_buf.setTransform(t_z_scale, 0, 0, t_z_scale, t_offset, t_offset);
		render();
