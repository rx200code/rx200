...
let view_a = -0;//toRad(-0);//-0;// Угол поворота окна карты, относительно карты(отрицательный угол поворота карты).
...
/** УПРАВЛЕНИЕ события и слушатели */
/** 
ОПИСАНИЕ всех действий.

1. Перемещение.
2. Зумирование, масштабирование.
3. Поворот.
4. Сброс поворота.
5. Меню.
...
ДЛЯ МЫШИ.
1. Перемещение.
1.1. Нажать любую из трех кнопок мыши и перемещать.

2. Зумирование, масштабирование.
2.1. Крутить колесико мыши.
2.2. Двойной клик левой кнопки, приблизить, двойной клик правой кнопки отдалить.

3. Поворот.
3.1. Нажать правую кнопку мыши + колесико мыши.

4. Сброс поворота.
4.1. Нажатие средний кнопки мыши, которая с колесиком.
...
*/
view.oncontextmenu = e => e.preventDefault();// Убираем контекстное меню.
...
let lastClickTime = 0;
const DOUBLE_CLICK_DELAY = 300; // задержка в мс

/*
https://www.google.com/search?q=%D0%BA%D0%B0%D0%BA%D0%BE%D0%B9+%D1%84%D0%B8%D0%BB%D1%8C%D0%BC+%D0%BF%D0%BE%D1%81%D0%BE%D0%B2%D0%B5%D1%82%D1%83%D0%B5%D1%88%D1%8C&sca_esv=8246e02b3da205e5&udm=50&fbs=AIIjpHymv3G69TjiKhuZ0DfWcgn1vOUgB19WwRlLMd_DBMe48fKjSFjk6qPhh1W1R10pWEOc_gWJ_luTddZU_c4oAV4EOHTObrj7jn3G-yLg5HMTWdcZHrKEysOG5CnsqF7d8e1AD1QmjSMsxM5K9359ZjkiAa8v8sDxzJwRt1ZFHcQZ3MVzfKeY2KOC0usG7Bmd970u9s4ZQYLHx89M2uEe0sMzrFSlHArUIY14_tPQjiaITCg6OeUfEuYkUN5XEdffW7FYcHbCmB5awZ3d16NR-67bilz1Cg&aep=1&ntc=1&sa=X&ved=2ahUKEwi3naj3_vmRAxW9CBAIHbD5NRQQ2J8OegQIEhAE&biw=1920&bih=955&dpr=1&mstk=AUtExfA5me98QfM7yK15lJ7Pis1SFf463-vcstpqfINuYB40inItzI2OY8J5Qaw7IM9b3gbE_DF_ZNP7JqDOnviQWfI5SS_HlcprWwouCUmhH-xwuTopZg4-VCl2BLBo7QSf-9tFo0-5JBaEpngWnq623xcqD_Fy_XfTwNw&csuir=1

Свидетель обвинения
харакири
*/

let click_timer_id = null;
let down_x, down_y;
let flag_click = true;

view.onpointerdown = e => {// нажатие мыши или прикосновение пальцем.
	view.setPointerCapture(e.pointerId);// Привязываем событие к элементы, чтоб не терять его при выходе за элемент в движении. // Отвяжется само.
	
	active_pointers.set(e.pointerId, e);// Сохраняем для мульти тача.
	
	
	if(active_pointers.size === 1){// одно касание или нажатие мыши.
		...
		if(timeDiff < DOUBLE_CLICK_DELAY && timeDiff > 40){// двойной клик. приближение на 1 зум
			
			clearTimeout(click_timer_id);
			lastClickTime = currentTime;// 0
			flag_click = false;
			...
			if(e.buttons === 2)zoom--;//Для совместимости браузеров, от дельты берем только знак.
			else zoom++;//Для совместимости браузеров, от дельты берем только знак.
		...
		if(e.buttons === 4){// Кнопка с колоесиком // Возвращаем угол повортоа в ноль.
			flag_click = false;
			view_a = -0;
			view_sin_a = 0;
			view_cos_a = 1;
	...
	}else if(active_pointers.size === 2){// 2 ТАЧЬ.
...
const MOVE_THRESHOLD_SQ = 25; // Порог в пикселях в квадрате.

let menu_pos = () => {
	if(confirm(toLat(rot_y) + ", " + toLon(rot_x) + "\n\nСБРОСИТЬ УГОЛ ПОВОРОТА КАРТЫ?\n" + toDeg(-view_a).toFixed(2))){
		view_a = -0;
		view_sin_a = 0;
		view_cos_a = 1;
...
view.onpointerup = e => {
	...
	const dx = e.offsetX - down_x;
	const dy = e.offsetY - down_y;
	if(flag_click && view_a !== 0 && (dx * dx + dy * dy) <= MOVE_THRESHOLD_SQ){
		
		const setTime = DOUBLE_CLICK_DELAY - (Date.now() - lastClickTime);
		info(lastClickTime);
		click_timer_id = setTimeout(menu_pos, setTime);
	}else flag_click = true;
	
	
	//out("onpointerup", getLat(), getLon());// TEST
	if(e.buttons === 0)active_pointers.delete(e.pointerId);
	out(e.pointerId, e.isPrimary, e.offsetX, e.offsetY, "UP", active_pointers.size, e.buttons);
	
	for(let e of active_pointers.values()){//e.isPrimary
		// Смещение точки вращения относительно центра в тайлах.
		center_to_pivot_x = (e.offsetX - viev_center_x) / tile_scale;
		center_to_pivot_y = (e.offsetY - viev_center_y) / tile_scale;
...
//canvas_buf.style.willChange = 'transform';
...
view.onpointermove = e => {
	if(active_pointers.has(e.pointerId)){
		...
		if(active_pointers.size === 1){
			...
			//canvas.style.transform = `translate(${e.offsetX - viev_center_x}px, ${canvas.offsetTop + e.movementY}px)`;
			//return;
			...
			const pointers = Array.from(active_pointers.values());
			// Пример: расчет расстояния между точками pointers[0] и pointers[1]
			
			let view_touch_distance =  Math.hypot(pointers[0].offsetX - pointers[1].offsetX, pointers[0].offsetY - pointers[1].offsetY) / dpr;// / tile_scale;
			...
			out(toDeg(new_angle), new_zoom, " 2 ТАЧЬ.");
...
view.onwheel = e => {// ПРОБНЫЙ ВАРИАНТ? для тестов
	/*
	Значения e.buttons в событии wheel:
	0: Кнопки не нажаты.
	1: Зажата левая кнопка мыши.
	2: Зажата правая кнопка мыши.
	4: Зажато само колесико (средняя кнопка).
	5: Зажаты одновременно левая кнопка и колесико (1 + 4).
	*/
	e.preventDefault(); // Важно, чтобы не скроллилась страница
	// if(e.deltaX) return; // Определение тачпада и отсеивание его, так как в нем прокрутк когда два пальца на экране.
	...
	if(e.buttons === 2){
		...
		out(toDeg(-view_a));
		return;
	}else if(e.buttons === 1){// Масштабирование.
		flag_click = false;
		// разница между курсором и центром
		let dx = e.offsetX - viev_center_x;
		let dy = e.offsetY - viev_center_y;
...
/** РЕНДЕР главная функция отображения. */
...
/*
	Для правильной работы РЕНДЕРА должны быть установлены следующие переменные:
	
	// Положение центра, относительно которого и будет построена карта. // Устанавливается почти всегда, кроме вращения вокруг центра.
	center_x
	center_y
	
	// Ширена и высота окна карты в тайлах учитывая масштаб. // Устанавливается при смене масштаба, или зумма.
	view_width_tile
	view_height_tile
	
	// Смещение смещение центра, тоесть половина высоты и ширены экрана в тайлах учитывая масштаб. // Устанавливается при смене масштаба, или зумма.
	viev_center_x_tile
	viev_center_y_tile
	
	// Синус и косинус угла поворота окна карты. // Устанавливается при повороте.
	view_sin_a
	view_cos_a
	
	//// для трансформации системы координат Канвас. // Устанавливаются при изменении зумма, и вращении.
	
	// Синус и косинус угла поворота окна, умноженное на масштаб отображения.
	scale_sin_a 
	scale_cos_a
	
	// Смещение пространства координат Канвас.
	view_shift_x
	view_shift_y
	
	//// Переменные в последующих функциях вызываемых из РЕНДЕРА.
	
	// Положение центра в окне карты, тоесть половина ширены и высоты окна экрана. // Устанавливаются при изменении окна карты в браузере.
	viev_center_x
	viev_center_y
	
	// Размер тайла. // почти не меняется.
	tile_size
	
	// Количество тайлов по любой из осей. // Меняется при изменении базового зумма.
	max_xy_tile
...
const test_append_elm_v3 = (x, y) => {
	...
	x %= max_xy_tile;
	y %= max_xy_tile;
	if(x < 0)x += max_xy_tile;
	if(y < 0)y += max_xy_tile;
	/**/
