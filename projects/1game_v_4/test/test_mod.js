...
///######################################################
test_mod.execute = function(){//JSON.stringify(obj);
	/// test 3 функцияю
	this.flag3 = true;
	let f_button_3 = function(){
		if(this.flag3){
			this.flag3 = false;
			game.view.button_run.inactive();
		}else{
			this.flag3 = true;
			game.view.button_run.pressed();
		}
		
	}.bind(this);
	this.button_test_3 = this.create_button("test 3",f_button_3);
	this.div.append(this.button_test_3);
	/// test 2 функцияю
	this.flag = true;
	let f_button_2 = function(){
		if(this.flag){
			this.flag = false;
			game.view.button_run.clear();
		}else{
			this.flag = true;
			game.view.button_run.display();
		}
	...
	let f_create_input = function(elm){
		let input_r = document.createElement('input');
		input_r.type = "range";
		input_r.min = 0;
		input_r.max = 255;
		...
		input_r.oninput = function(){
			//document.getElementById('output').innerHTML = (this.value*1).toString(16);
			input_t.value = (this.value*1).toString(16);
		}
		input_t.oninput = function(){
			//document.getElementById('output').innerHTML = parseInt(this.value == '' ? '0' : this.value , 16);
			input_r.value = parseInt(this.value == '' ? '0' : this.value , 16);
		}
		
		
		elm.append(input_r,input_t,document.createElement('br'));
	}
	f_create_input(div);
	f_create_input(div);
	f_create_input(div);
	
	//div.append(h3,input_r);
...
<input
	...
	value="127"
	id="color_1"
	oninput="f_m_color((this.value*1).toString(16), 'c_1')"
>
<input
	type="text"
	size="2"
	id="c_1"
	value="7f"
	oninput="f_m_color(parseInt(this.value == '' ? '0' : this.value , 16), 'color_1')"
>
<br>
*/
