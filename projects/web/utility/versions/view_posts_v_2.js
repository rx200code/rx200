...
posts.forEach(function(post){
	...
	for(var i = 0; i < persons.length; i++){
		if(persons[i].id === animal.person_id){
			person_contact = "Контакты: " + persons[i].contacts.tel + " @" + persons[i].contacts.vk.slice(persons[i].contacts.vk.lastIndexOf('/') + 1) + "(" + persons[i].name +").";
			
			break;
		}
	}
	
	WScript.StdOut.WriteLine("person_contact: " + person_contact);
