...
// resource_domain = "ishydom.zoonovosibirsk"; signer_id = "7389836, 212189581, 385144770, 417306923, 671373562, 666960319, 673574032, 741537511"; offset = 1000;
var resource_domain = "ishydom.zoonovosibirsk";
var signer_id = "7389836, 212189581, 385144770, 417306923, 671373562, 666960319, 673574032, 741537511";
var offset = 1000;

/* CODE getIdsPostUserNotReposted
var result = [];
var signer_ids = [signer_id];
var items = API.wall.get({"domain":resource_domain, "offset":offset,"count": 100}).items;
var i = 0;
while(i < items.length){
	if(signer_ids.indexOf(items[i].signer_id) != -1)result.push(items[i].id);
	i = i + 1;
};
return result;
//
'var result = [];var signer_ids = [' + signer_id + '];var items = API.wall.get({"domain":"' + resource_domain + '", "offset":' + offset + ',"count": 100}).items;var i = 0;while(i < items.length){if(signer_ids.indexOf(items[i].signer_id) != -1)result.push(items[i].id);i = i + 1;};return result;';
// END CODE */
...
function search_post(){
	var code = 'var result = [];var signer_ids = [' + signer_id + '];var items = API.wall.get({"domain":"' + resource_domain + '", "offset":' + offset + ',"count": 100}).items;var i = 0;while(i < items.length){if(signer_ids.indexOf(items[i].signer_id) != -1)result.push(items[i].id);i = i + 1;};return result;';
...
while(true){
	...
	WScript.StdOut.WriteLine("signer_id: " + signer_id);
	...
	WScript.StdOut.WriteLine("Найдено: " + ids_posts.length + " постов с совподающими идами [" + signer_id + "].");
