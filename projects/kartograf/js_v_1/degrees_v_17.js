function Degrees(){
	...
	let grid = () => {
		...
		let path_1 = createElementNS("path", ["fill", "none"], ["stroke-width", 4], ["stroke-opacity", .5], ["stroke", "#800"]);// Если маршрут разорван то первая часть отобразится красным.
		svg.appendChild(path_1);
		
		b_in_track.onclick = () => {
			let arr_1 = [];// Запасной если маршрут рваный, может быть при dop_r = true;
			let arr = [];
			let counter = 0;
			let f_push_i = i => {
				arr.push(arr_c_g[i]);
			...
			// способ решения исключений, через повторные точки, или через дополнительные маршрутя.
			let dop_r = false;
			...
			// Вывод маршрута.
			let d = "";
			if(arr_1.length > 1)d += "M"+arr_1[0][0]+","+arr_1[0][1];
			for(i = 1; i < arr_1.length; i++)d += "L"+arr_1[i][0]+","+arr_1[i][1];
			path_1.setAttributeNS(null, "d", d);
			d = "";
			if(arr.length > 1)d += "M"+arr[0][0]+","+arr[0][1];
			for(i = 1; i < arr.length; i++)d += "L"+arr[i][0]+","+arr[i][1];
			path.setAttributeNS(null, "d", d);
			document.getElementById("test_grid").textContent = counter;
		};
