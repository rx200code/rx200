function M_load(){
	...
	b_save.onclick = async () => {
		if(save_check.checked){
			...
			if(document.getElementById('active_map_n').checked && map.get_layer_n().getSource() !== null)await f_l_to_can(map.get_layer_n());// Рисуем дополнительный слой если есть.
			...
			// 3. Сетка.
			degrees.f_draw_grid.f_load(ctx, w_p_true, h_p_true, res_true, coor_t_l, coor_b_r, w_t_0, h_t);
