function Degrees(){
	...
	let f_number_in_format_d = (n, east) => {
		...
		}else if(format_d == 8)text += Math.round(n / 1000)+"km";
		else if(format_d == 9)text += Math.round(n)+"m";
		else if(format_d == 10)text += n.toFixed(2)+"m";
		else if(format_d == 11)text += Math.round(n / 0.3048006096)+"ft";// Геодезический фут 0.3048006096 метра.
		else if(format_d == 12)text += Math.round(n / 0.3048)+"ft";// Международный фут 0.3048 метра.
		else if(format_d == 13)text += Math.round(n / 0.9144)+"yd";// Ярд 0.9144 метра.
		else if(format_d == 14)text += Math.round(n / 1609.344)+"mile";// Миля 1609.344 метра.
		else if(format_d == 15)text += Math.round(n / 1855.3248465545596)+"gm";// Географическая миля 1855.3248465545596 метра, вычесленная для WGS 84.
		else if(format_d == 16)text += Math.round(n / 1852)+"nm";// Морская миля 1852 метра.
		else text += "Error";
		return text;
	};
	...
	let f_coor_transform = to_wgs_84_deg;
	let f_sys_c = () => {
		let sys_c = document.getElementById("sys_c").selectedIndex - 1;
		f_coor_transform = sc[sys_c].f_to;
		if(sc[sys_c].metre)f_to_xy();
		else f_to_fl();
	}
	...
	let f_move = () => {
		...
		//if(document.getElementById("draw_grid_2").checked)draw_grid(2);
	};
	let out_text_z = c => c[3] ? "z:"+c[2]+" S": "z:"+c[2]+" N";
	let f_out_angles_text = () => {
		// Переводим координаты в нужную систему/проекцию координат
		let c_top_left_out = f_coor_transform(coor_top_left);
		let c_top_right_out = f_coor_transform(coor_top_right);
		let c_bot_left_out = f_coor_transform(coor_bot_left);
		let c_bot_right_out = f_coor_transform(coor_bot_right);
	...
	let path_grid_1 = createPathGrid();
	
	path_grid_1.setAttributeNS(null, "stroke-width", 1);
	path_grid_1.setAttributeNS(null, "stroke", "#000");
	//path_grid_1.setAttributeNS(null, "stroke-linecap", "round");
	path_grid_1.setAttributeNS(null, "stroke-opacity", .5);
	svg.appendChild(path_grid_1);
	
	let path_grid_2 = createPathGrid();
	
	path_grid_2.setAttributeNS(null, "stroke-width", 3);
	path_grid_2.setAttributeNS(null, "stroke", "#f00");
	path_grid_2.setAttributeNS(null, "stroke-linecap", "round");
	path_grid_2.setAttributeNS(null, "d", "M"+(width_map / 2)+","+(height_map / 2)+"h0");
	//svg.appendChild(path_grid_2);
	// 2. Создаем установку цвета.
	...
	let draw_grid = function(id){
		...
		//Test
		//path_grid_1.setAttributeNS(null, "stroke", document.getElementById("grid_color_"+id).value);
		...
		let id_sys_grid_1 = document.getElementById("sys_grid_1").selectedIndex - 1;
		
		let f_to = sc[id_sys_grid_1].f_to;
		let f_from = sc[id_sys_grid_1].f_from;
		
		let coor_centr_to = f_to(coor_centr);
		let z_s = coor_centr_to.slice(2);
		
		let d_path = "";
		let interval = sc[id_sys_grid_1].metre ? 300000: toRad(10);
		
		// Находим координаты углов в нужной проекции.
		...
		let left = c_top_left_to[0] < c_bot_left_to[0] ? c_top_left_to[0]: c_bot_left_to[0];
		
		
		//
		let test_c_left = f_to([coor_top_left[0], (coor_bot_right[1] - coor_top_left[1]) / 2 + coor_top_left[1]].concat(z_s));
		left = left < test_c_left[0] ? left: test_c_left[0];
		
		
		left -= left % interval;
		let right = c_top_right_to[0] > c_bot_right_to[0] ? c_top_right_to[0]: c_bot_right_to[0];
		
		
		//
		let test_c_right = f_to([coor_top_right[0], (coor_bot_right[1] - coor_top_left[1]) / 2 + coor_top_left[1]].concat(z_s));
		right = right > test_c_right[0] ? right: test_c_right[0];
		
		
		right -= right % interval;
		let top = c_top_right_to[1] > c_top_left_to[1] ? c_top_right_to[1]: c_top_left_to[1];
		
		
		let top_3 = coor_top_right[1];
		
		//
		let test_c_top = f_to([(coor_bot_right[0] - coor_top_left[0]) / 2 + coor_top_left[0], coor_top_right[1]].concat(z_s));
		top = top > test_c_top[1] ? top: test_c_top[1];
		let top_2 = top;//c_top_right_to[1] > c_top_left_to[1] ? c_top_right_to[1]: c_top_left_to[1];
		
		top -= top % interval;
		let bot = c_bot_left_to[1] < c_bot_right_to[1] ? c_bot_left_to[1]: c_bot_right_to[1];
		
		//
		let test_c_bot = f_to([(coor_bot_right[0] - coor_top_left[0]) / 2 + coor_top_left[0], coor_bot_right[1]].concat(z_s));
		bot = bot < test_c_bot[1] ? bot: test_c_bot[1];
		
		
		let bot_2 = bot;//c_bot_left_to[1] < c_bot_right_to[1] ? c_bot_left_to[1]: c_bot_right_to[1];
		
		let bot_3 = coor_bot_right[1];
		
		bot -= bot % interval;
		// на один шаг сдвигаем значения.
		//*
		left -= interval;
		right += interval;
		top += interval;
		document.getElementById("test_out").innerHTML = toDeg(top)+"<br>";// 90 deg так же проблемы если больше 90 градусов.
		bot -= interval;
		//*/
		// Определяем выходит ли за пределы.
		/*
		let c_WM_4 = f_from([c_top_right_to[1] < c_top_left_to[1] ? right: left, top].concat(z_s));
		if(c_WM_4[1] < top_3 || c_WM_4[1] > 20037508 || isNaN(c_WM_4[1])){
			document.getElementById("test_out_2").innerHTML = "<br>Yes "+c_WM_4[1]+"<br>t_3 "+top_3;
			//top -= interval;
		}else document.getElementById("test_out_2").innerHTML = "<br>Not "+c_WM_4[1]+"<br>t_3 "+top_3;
		// bot
		let c_WM_5 = f_from([c_bot_left_to[1] > c_bot_right_to[1] ? right: left, bot].concat(z_s));
		if(c_WM_5[1] > bot_3 || c_WM_5[1] < -20037508 || isNaN(c_WM_5[1])){
			document.getElementById("test_out_3").innerHTML = "<br>Yes";
			//bot += interval;
		}else document.getElementById("test_out_3").innerHTML = "<br>Not";
		//*/
		/*
		let c_WM_4 = f_from([c_top_right_to[1] > c_top_left_to[1] ? right: left, top].concat(z_s));
		if(c_WM_4[1] < top_3 )document.getElementById("test_out_2").innerHTML = "<br>Yes top_2";// Требуется определять полушарие N S
		else if(c_WM_4[1] > 20037508)document.getElementById("test_out_2").innerHTML = "<br>Yes 20037508";
		else if(isNaN(c_WM_4[1]))document.getElementById("test_out_2").innerHTML = "<br>Yes NaN";
		else document.getElementById("test_out_2").innerHTML = "<br>Not";
		// bot
		let c_WM_5 = f_from([c_bot_left_to[1] > c_bot_right_to[1] ? right: left, bot].concat(z_s));
		if(c_WM_5[1] > bot_3)document.getElementById("test_out_3").innerHTML = "<br>Yes bot_2";// Требуется определять полушарие N S
		else if(c_WM_5[1] < -20037508)document.getElementById("test_out_3").innerHTML = "<br>Yes -20037508";
		else if(isNaN(c_WM_5[1]))document.getElementById("test_out_3").innerHTML = "<br>Yes NaN";
		else document.getElementById("test_out_3").innerHTML = "<br>Not";
		//*/
		/*
		let c_WM_2 = f_from([c_top_right_to[1] > c_top_left_to[1] ? c_top_right_to[0]: c_top_left_to[0], top].concat(z_s));
		let c_WM_3 = f_from([c_bot_left_to[1] < c_bot_right_to[1] ? c_bot_left_to[0]: c_bot_right_to[0], bot].concat(z_s));
		
		if(c_WM_2[1] < top_2 || c_WM_2[1] > 20037508 || isNaN(c_WM_2[1]))document.getElementById("test_out_2").innerHTML = "<br>Yes";
		else document.getElementById("test_out_2").innerHTML = "<br>Not";
		
		document.getElementById("test_out_3").innerHTML = "<br>WM_top = "+c_WM_2[1]+"<br>WM_bot = "+c_WM_3[1];
		//*/
		
		document.getElementById("test_out_4").innerHTML = "<br>left = "+toDeg(left)+"<br>right = "+toDeg(right)+"<br>top = "+toDeg(top)+"<br>bot = "+toDeg(bot);
		...
		for(let y = bot; y <= top + (interval * .1); y += interval){
			coor[1] = y;
			i = 0;
			//*
			if(coor[1] < bot_2)coor[1] = bot_2;
			else if(coor[1] > top_2)coor[1] = top_2;
			//*/
			/* Для wgs84
			let test_y = 89.9
			if(coor[1] > toRad(test_y))coor[1] = toRad(test_y);
			if(coor[1] < toRad(-test_y))coor[1] = toRad(-test_y);
			//*/
			counter = 0;
			for(let x = left; x <= right + (interval * .1); x += interval){
				counter++;
				//if(counter > 500){alert("left = "+left+"\nrigth = "+right+"\ninterval = "+interval+"\nz = "+z_s);return;}
				...
				//if(x == left && y == top)alert();
				
				let c_pixel = map.map.getPixelFromCoordinate(c_WM);
				
				//if(isNaN(c_pixel[0]))document.getElementById("test_out").innerHTML = coor.concat(z_s).map(toDeg)+"<br>";// 90 deg
				
				d_path += i == 0 ? "M"+c_pixel[0]+","+c_pixel[1]: "L"+c_pixel[0]+","+c_pixel[1];
		...
		path_grid_1.setAttributeNS(null, "d", d_path+line_h_arr.join(''));
		
		
		//document.getElementById("test_out_2").innerHTML = d_path;
		
		//document.getElementById("test_out_3").innerHTML = "<br>"+line_h_arr.join('<br>');
		
		
		//let tt = map.map.getPixelFromCoordinate(coor_centr);
		//path_grid_1.setAttributeNS(null, "d", "M"+tt[0]+","+tt[1]+"h0,0");
		...
		//document.getElementById("test_out_2").innerHTML = document.getElementById("grid_color_"+id).value;
	};
	...
	menu.onchange = function(e){
		if(e.target.id == "markings_corners"){
			if(document.getElementById("markings_corners").checked){
				f_sys_c();
				svg.appendChild(degrees_text);
				document.querySelector(".ol-attribution").style.display = "none";
				f_move();
			}else{
				if(svg.contains(degrees_text)){
					svg.removeChild(degrees_text);
					document.querySelector(".ol-attribution").style.display = "inline-block";
				}
			}
		}else if(e.target.id == "format_d")format_d = document.getElementById("format_d").selectedIndex;
		else if(e.target.id == "sys_c")f_sys_c();
		if(document.getElementById("markings_corners").checked){
			f_out_angles_text();
		}
	};
