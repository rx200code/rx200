function M_load(){
	...
	g_zoom.insertAdjacentHTML('beforeend', "<br><br>Chrome, Edge, Opera max 16384x16384. Firefox max ≈ 11000x11000<br>width : ");
	...
	let r_p = 2000;// Для тестов достаточно 1000;//4000;// Разрешение стороны в пикселях, оноже в квадрате будет показывать общее количество пикселей в карте.
	let f_zoom = () => {
		if(auto_z.checked){
	...
	let f_l_to_can = layer => {
		return new Promise(async resolve => {
			...
			// Если проекция яндекса.
			let s_t_h, h_tt;
			let dop_flg = false;// для ggc_500.
			if(source.getProjection().getCode() === "yndex"){
				let h_yndex = to_M(coor_t_l)[1];
				s_t_h = (width_WM - (h_yndex + width_WM / 2)) / w_title_WM | 0;// start_title_height //
				h_tt = -Math.round(((width_WM - (h_yndex + width_WM / 2)) % w_title_WM) / res) * mul_s;
			}else if(source.getProjection().getCode() === "ggc_500"){// для ggc_500.
				let h_ggc_500 = -coor_t_l[1];
				dop_flg = true;
				s_t_h = (width_WM - (h_ggc_500 + width_WM / 2)) / w_title_WM | 0;// start_title_height //
				h_tt = -Math.round((w_title_WM - ((width_WM - (h_ggc_500 + width_WM / 2)) % w_title_WM)) / res) * mul_s;
			}else{
				s_t_h = (width_WM - (ex[3] + width_WM / 2)) / w_title_WM | 0;// start_title_height //
				h_tt = -Math.round(((width_WM - (ex[3] + width_WM / 2)) % w_title_WM) / res) * mul_s;
			}
			...
			// Если Esri Hillshade и слой надо сделать прозрачным.
			let ersi_h_flg = false;
			if(source.getUrls()[0] === 'https://server.arcgisonline.com/arcgis/rest/services/Elevation/World_Hillshade/MapServer/tile/{z}/{y}/{x}')ersi_h_flg = true;
				...
				await new Promise(resolve => {
					...
					img.onload = function(){
						...
						if(ersi_h_flg){
							...
							ctx_h.drawImage(img, 0, 0, 256, 256);
							...
							ctx.drawImage(can_h, coor_title[0], coor_title[1], s_p * mul_s, s_p * mul_s);
						}else ctx.drawImage(img, coor_title[0], coor_title[1], s_p * mul_s, s_p * mul_s);
						img.remove();
						flag_one = true;
						resolve(true);
					};
				...
				for(let w = s_t_w; w <= e_t_w; w++){
					if(flag_one)flag_one = false;
						...
						promises.push(new Promise(resolve => {
							...
							if(dop_flg)tileCoord[2] -= dop_counter * 2;// для ggc_500.
							let img = new Image();
							let coor_title = [];
							coor_title[0] = w_ttt;
							coor_title[1] = h_tt;
	...
	let f_save = async () => {
		...
		if(document.getElementById("s_check_pdf").checked){
			...
			let url = can.toDataURL("image/jpeg", b_quality.valueAsNumber);
			pdf.addImage(url, 'JPEG', 0, 0, w_p_true, h_p_true);
			pdf.save('map.pdf');
			window.URL.revokeObjectURL(url);
		...
		if(document.getElementById("s_check_pdfA4").checked){
			...
			let w_A4 = 297;
			let h_A4 = 210;
			let dpi = document.getElementById("s_dpi").value;
			// px
			let px_res = 25.4 / dpi;
			let w_A4_p = w_A4 / px_res;
			let h_A4_p = h_A4 / px_res;
			// 2. Вычисляем отступы, и наклон, 'p' или 'l'.
			let sum_1 = Math.ceil(w_p_true / w_A4_p) * Math.ceil(h_p_true / h_A4_p);
			let sum_2 = Math.ceil(w_p_true / h_A4_p) * Math.ceil(h_p_true / w_A4_p);
			let orientation, w_c, h_c, w_indent, h_indent, w_mm, h_mm;
			if(sum_1 < sum_2 || (sum_1 === sum_2 && Math.abs((w_A4_p - (w_p_true % w_A4_p)) - (h_A4_p - (h_p_true / h_A4_p))) < Math.abs((h_A4_p - (w_p_true % h_A4_p)) - (w_A4_p - (h_p_true / w_A4_p))))){
				orientation = "l";// Лист А4 горизонтально.
				// Количество листов по w и h.
				w_c = Math.ceil(w_p_true / w_A4_p);
				h_c = Math.ceil(h_p_true / h_A4_p);
				// отступы в mm
				w_indent = ((w_A4_p - (w_p_true % w_A4_p)) / w_c) / 2 * px_res;
				h_indent = ((h_A4_p - (h_p_true % h_A4_p)) / h_c) / 2 * px_res;
				// ширена и высота изображения в mm.
				w_mm = w_A4 - w_indent * 2;
				h_mm = h_A4 - h_indent * 2;
			}else{
				orientation = "p";
				w_c = Math.ceil(w_p_true / h_A4_p);
				h_c = Math.ceil(h_p_true / w_A4_p);
				w_indent = ((h_A4_p - (w_p_true % h_A4_p)) / w_c) / 2 * px_res;
				h_indent = ((w_A4_p - (h_p_true % w_A4_p)) / h_c) / 2 * px_res;
				w_mm = h_A4 - w_indent * 2;
				h_mm = w_A4 - h_indent * 2;
			}
			// Шаг по картинке в пикселях.
			let w_step = w_p_true / w_c;
			let h_step = h_p_true / h_c;
			...
			let can_save = document.createElement("canvas");
			can_save.setAttribute("width", w_step);
			can_save.setAttribute("height", h_step);
			let ctx_s = can_save.getContext("2d");
			for(let i = 0; i < w_c; i++){
				for(let j = 0; j < h_c; j++){
					ctx_s.putImageData(ctx.getImageData(w_step * i, h_step * j, w_step * (i + 1), h_step * (j + 1)), 0, 0);
					let url = can_save.toDataURL("image/jpeg", b_quality.valueAsNumber);
					if(i !== 0 || j !== 0)pdf.addPage("a4", orientation);
					pdf.addImage(url, 'JPEG', w_indent, h_indent, w_mm, h_mm);
					window.URL.revokeObjectURL(url);
				}
			}
			pdf.save('map_A4.pdf');
			test_out.textContent = "В pdf сохранено "+(w_c * h_c)+" листов.";
		}
