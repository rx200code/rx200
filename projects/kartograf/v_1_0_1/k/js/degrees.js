function Degrees(){
	...
	for(let i = 0; i < sc.length; i++){
		...
		op.setAttribute("title", sc[i].title);
	...
	let f_out_angles_text = () => {
		...
		if(sc[sys_c_ind].center){
			...
			centr_op = sc[sys_c_ind].f_to([(coor_bot_right[0] + coor_top_left[0]) / 2, (coor_bot_right[1] + coor_top_left[1]) / 2]).slice(2);
		}
	...
	let f_out_t_load_a = (ctx, x, y, w_t, h_t, t, t2) => {
		ctx.fillStyle = "#ffffff88";
		ctx.fillRect(x, y, w_t, h_t * 2);
		ctx.fillStyle = "#000000ff";
		ctx.fillText(t, x, y + h_t);
		ctx.fillText(t2, x, y + h_t * 2);
	};
	this.f_load_a_t = (ctx, ctx_w, ctx_h, coor_top_left, coor_bot_right, w_t_0, h_t) => {// Функция для установки текста в углах сохраняемой карты.
		if(document.getElementById("markings_corners").checked){
			let centr_op = [];
			if(sc[sys_c_ind].center){
				if(sc[sys_c_ind].angle)hotine_a = value_angle;
				centr_op = sc[sys_c_ind].f_to([(coor_bot_right[0] + coor_top_left[0]) / 2, (coor_bot_right[1] + coor_top_left[1]) / 2]).slice(2);
			...
			// Переводим координаты в нужную систему/проекцию координат
			let c_top_left_out = sc[sys_c_ind].f_to([...coor_top_left, ...centr_op]);
			let c_top_right_out = sc[sys_c_ind].f_to([coor_bot_right[0], coor_top_left[1], ...centr_op]);
			let c_bot_left_out = sc[sys_c_ind].f_to([coor_top_left[0], coor_bot_right[1], ...centr_op]);
			let c_bot_right_out = sc[sys_c_ind].f_to([...coor_bot_right, ...centr_op]);
			// Переводим в нужный формат и отображаем координаты.
			let z = sc[sys_c_ind].zone;// Истина если есть функция зоны.
			let a = sc[sys_c_ind].f_a ? " ∠"+toDeg(sc[sys_c_ind].f_a(c_top_left_out)).toFixed(2)+"°": "";
			let t_top = f_number_in_format_d(c_top_left_out[1], false)+(z ? " "+f_out_text_z(c_top_left_out): "");
			let t_bot = f_number_in_format_d(c_top_left_out[0], true)+a;
			let w_t = Math.max(t_top.length, t_bot.length) * w_t_0;
			ctx.textAlign = "start";
			f_out_t_load_a(ctx, 0, 0, w_t, h_t, t_top, t_bot);
			t_top = f_number_in_format_d(c_top_right_out[1], false)+(z ? f_out_text_z(c_top_right_out): "");
			t_bot = f_number_in_format_d(c_top_right_out[0], true);
			w_t = Math.max(t_top.length, t_bot.length) * w_t_0;
			f_out_t_load_a(ctx, ctx_w - w_t, 0, w_t, h_t, t_top, t_bot);
			t_top = f_number_in_format_d(c_bot_left_out[1], false)+(z ? " "+f_out_text_z(c_bot_left_out): "");
			t_bot = f_number_in_format_d(c_bot_left_out[0], true);
			w_t = Math.max(t_top.length, t_bot.length) * w_t_0;
			f_out_t_load_a(ctx, 0, ctx_h - h_t * 2 - 1, w_t, h_t + .5, t_top, t_bot);
			t_top = f_number_in_format_d(c_bot_right_out[1], false)+(z ? f_out_text_z(c_bot_right_out): "");
			t_bot = f_number_in_format_d(c_bot_right_out[0], true);
			w_t = Math.max(t_top.length, t_bot.length) * w_t_0;
			f_out_t_load_a(ctx, ctx_w - w_t, ctx_h - h_t * 2 - 1, w_t, h_t + .5, t_top, t_bot);
			// Если есть точка в центре добавляем.
			if(document.getElementById("p_center").checked){
				ctx.fillStyle = "#0000ffff";
				ctx.beginPath();
				ctx.arc(ctx_w / 2, ctx_h / 2, 2.5, 0, Math.PI * 2);
				ctx.fill();
			}
		}
	};
	...
	let arr_c_y = [];
	let a_f_grid = 0;
	let f_grid = (interval, coor_top_l, coor_bot_r, sc, wm = false, plus = true) => {
		// Находим центр
		...
		while(bot <= top){
			...
			for(let x = left; x <= right; x += interval){
				...
				coor[0] = x;
				arr_c_grid[i] = wm ? sc.f_from(coor): map.map.getPixelFromCoordinate(sc.f_from(coor));
				i++;
			}
			bot += interval;
		}
	...
	let out_t_load_m = (side, t, c, ctx, w_t_0, h_t, w_or_h) => {
		switch(side){
			case "l":
				ctx.fillStyle = "#ffffff88";
				ctx.fillRect(0, c - (h_t / 2), w_t_0 * t.length, h_t);
				ctx.fillStyle = "#000000ff";
				ctx.textAlign = "left";
				ctx.fillText(t, 0, c + (h_t / 2));
				break;
			case "r":
				ctx.fillStyle = "#ffffff88";
				ctx.fillRect(w_or_h - w_t_0 * t.length, c - (h_t / 2), w_t_0 * t.length, h_t);
				ctx.fillStyle = "#000000ff";
				ctx.textAlign = "right";
				ctx.fillText(t, w_or_h, c + (h_t / 2));
				break;
			case "t":
				ctx.fillStyle = "#ffffff88";
				ctx.fillRect(c - (w_t_0 * t.length / 2), 0, w_t_0 * t.length, h_t);
				ctx.fillStyle = "#000000ff";
				ctx.textAlign = "center";
				ctx.fillText(t, c, h_t);
				break;
			case "b":
				ctx.fillStyle = "#ffffff88";
				ctx.fillRect(c - (w_t_0 * t.length / 2), w_or_h - h_t - 1, w_t_0 * t.length, h_t + 1);
				ctx.fillStyle = "#000000ff";
				ctx.textAlign = "center";
				ctx.fillText(t, c, w_or_h - 1);
				break;
		}
	};
	function marking(options){
		...
		///
		this.f_load = (ctx, ctx_w, ctx_h, res, coor_top_left, coor_bot_right, w_t_0, h_t) => {// Функция для прописовки сетки разметки при сохранении.
			if(input_on.checked){
				if(sc[id_sc].angle)hotine_a = angle;
				f_grid(interval, coor_top_left, coor_bot_right, sc[select_cs.selectedIndex - 1], true);
				// Переводим координаты ВМ в координаты картинки.
				arr_c_grid.forEach(c => {
					c[0] = (c[0] - coor_top_left[0]) / res;
					c[1] = (coor_top_left[1] - c[1]) / res;
				});
				// Устанавливаем цвет, прозрачность.
				ctx.lineWidth = 1;
				let opacity = (input_opacity.valueAsNumber * 255) | 0;
				ctx.strokeStyle = input_color.value+(opacity <= 0xF ? "0": "")+opacity.toString(0x10);
				
				// рисуем линии разметки.
				ctx.beginPath();
				for(let i = w_grid; i < arr_c_grid.length - w_grid; i++){
					if(i % w_grid === 0)ctx.moveTo(arr_c_grid[i][0], arr_c_grid[i][1]);
					else ctx.lineTo(arr_c_grid[i][0], arr_c_grid[i][1]);
				}
				for(let i = 1; i < w_grid - 1; i++){
					for(let j = i; j < arr_c_grid.length; j += w_grid){
						//d_path += (j == i ? "M": "L")+arr_c_grid[j][0]+","+arr_c_grid[j][1];
						if(j === i)ctx.moveTo(arr_c_grid[j][0], arr_c_grid[j][1]);
						else ctx.lineTo(arr_c_grid[j][0], arr_c_grid[j][1]);
					}
				}
				ctx.stroke();
				...
				//*
				if(input_t.checked){// Рисуем тексты по бокам.
					let offset = 1;
					let flag_l = false;
					let flag_r = false;
					let flag_d = false;// Потом добавить условие, например от 50 градусов.
					...
					for(let i = w_grid; i < arr_c_grid.length - w_grid; i++){
						if(i % w_grid !== 0){
							if(!flag_l){
								c = intersection(arr_c_grid[i - 1][0], arr_c_grid[i - 1][1], arr_c_grid[i][0], arr_c_grid[i][1], offset, ctx_h);
								if(c !== 0){
									out_t_load_m("l", f_format_t_b(arr_c_y[i / w_grid | 0], false, select_step.selectedIndex), c, ctx, w_t_0, h_t);
									flag_l = true;
								}else if(flag_d){
									c = intersection(arr_c_grid[i - 1][1], arr_c_grid[i - 1][0], arr_c_grid[i][1], arr_c_grid[i][0], offset, ctx_w);
									if(c !== 0){
										out_t_load_m("t", f_format_t_b(arr_c_y[i / w_grid | 0], true, select_step.selectedIndex), c, ctx, w_t_0, h_t);
										flag_l = true;
									}
								}
							}
							if(!flag_r){
								c = intersection(arr_c_grid[i - 1][0], arr_c_grid[i - 1][1], arr_c_grid[i][0], arr_c_grid[i][1], ctx_w - offset, ctx_h);
								if(c !== 0){
									out_t_load_m("r", f_format_t_b(arr_c_y[i / w_grid | 0], false, select_step.selectedIndex), c, ctx, w_t_0, h_t, ctx_w);
									flag_r = true;
								}else if(flag_d){
									c = intersection(arr_c_grid[i - 1][1], arr_c_grid[i - 1][0], arr_c_grid[i][1], arr_c_grid[i][0], ctx_h - offset, ctx_w);
									if(c !== 0){
										out_t_load_m("b", f_format_t_b(arr_c_y[i / w_grid | 0], true, select_step.selectedIndex), c, ctx, w_t_0, h_t, ctx_h);
										flag_r = true;
									}
								}
							}
					...
					for(let i = 1; i < w_grid - 1; i++){
						for(let j = i; j < arr_c_grid.length; j += w_grid){
							if(j >= w_grid){
								if(!flag_t){
									c = intersection(arr_c_grid[j][1], arr_c_grid[j][0], arr_c_grid[j - w_grid][1], arr_c_grid[j - w_grid][0], offset, ctx_w);
									if(c !== 0){
										out_t_load_m("t", f_format_t_b(arr_c_x[i], true, select_step.selectedIndex), c, ctx, w_t_0, h_t);
										flag_t = true;
									}else if(flag_d){
										c = intersection(arr_c_grid[j][0], arr_c_grid[j][1], arr_c_grid[j - w_grid][0], arr_c_grid[j - w_grid][1], offset, ctx_h);
										if(c !== 0){
											out_t_load_m("l", f_format_t_b(arr_c_x[i], false, select_step.selectedIndex), c, ctx, w_t_0, h_t);
											flag_t = true;
										}
									}
								}
								if(!flag_b){
									c = intersection(arr_c_grid[j][1], arr_c_grid[j][0], arr_c_grid[j - w_grid][1], arr_c_grid[j - w_grid][0], ctx_h - offset, ctx_w);
									if(c !== 0){
										out_t_load_m("b", f_format_t_b(arr_c_x[i], true, select_step.selectedIndex), c, ctx, w_t_0, h_t, ctx_h);
										flag_b = true;
									}else if(flag_d){
										c = intersection(arr_c_grid[j][0], arr_c_grid[j][1], arr_c_grid[j - w_grid][0], arr_c_grid[j - w_grid][1], ctx_w - offset, ctx_h);
										if(c !== 0){
											out_t_load_m("r", f_format_t_b(arr_c_x[i], false, select_step.selectedIndex), c, ctx, w_t_0, h_t, ctx_w);
											flag_b = true;
										}
									}
								}
	...
	this.f_draw_1 = new marking({id_sc:1});
	this.f_draw_2 = new marking({color:"#ff5050"});
	///#####################################
	...
	//let grid = () => {
		...
		// точки треки маршруты.
		let select_save_d = createElement("select");
		elm = createElement("option", ["disabled", true]);
		elm.textContent = "сохраняем";
		select_save_d.append(elm);
		elm = createElement("option");
		elm.textContent = "все в маршруты";
		select_save_d.append(elm);
		elm = createElement("option");
		elm.textContent = "все в треки";
		select_save_d.append(elm);
		elm = createElement("option");
		elm.textContent = "точки";
		select_save_d.append(elm);
		elm = createElement("option");
		elm.textContent = "точки и маршруты";
		select_save_d.append(elm);
		elm = createElement("option");
		elm.textContent = "точки и треки + time";
		select_save_d.append(elm);
		elm = createElement("option");
		elm.textContent = "линии маршруты";
		select_save_d.append(elm);
		elm = createElement("option");
		elm.textContent = "линии треки";
		select_save_d.append(elm);
		select_save_d.selectedIndex = 5;
		save_s_addition.append(select_save_d);
		// Добавить нумирацию.
		save_s_addition.insertAdjacentHTML('beforeend', "<br><br>С нумирацией: ");
		let input_save_n = createElement("input", ["type", "checkbox"], ["checked", true]);
		save_s_addition.append(input_save_n);
		// знаков после запятой.
		save_s_addition.insertAdjacentHTML('beforeend', "<br>Знаков после запятой координат:");
		...
		let input_save_dop = createElement("input", ["type", "checkbox"], ["disabled", true]);
		...
		// ФУНКЦИЯ создания объекта gpx, для сохранения или импорта в треки.
		let f_in_gpx = () => {
			/// Основные параметры записи. Настройки.
			// Внутреннее пространство, или только края.
			let inner = true;//false;
			let flag_line = true;// Определяет есть ли вообще линии.
			if(select_save_d.selectedIndex > 3)inner = false;
			/// Формируем gpx.
			lim_d = input_seve_d.value | 0;// Лимит на знаки после запятой.
			if(input_save_n.checked){// Настраиваем нумерацию.
				if(flag_out_n){
					remainder_n_w = arr_c_g.length % w_g;// Определяем остаток если сработал лимит.
					h_g = (arr_c_g.length - remainder_n_w) / w_g;// определяем высоту
				}else{
			...
			let f_push_i;
			let f_push_i_1 = i => {
				let c = to_wgs_84_gpx(map.map.getCoordinateFromPixel(arr_c_g[i]));
				let n = arr.length;
				arr[n] = {lon:c[0], lat:c[1]};
				arr_WM[n] = from_wgs_84([toRad(c[0]), toRad(c[1])]);
			};
			let f_push_i_2 = i => {
				let c = to_wgs_84_gpx(map.map.getCoordinateFromPixel(arr_c_g[i]));
				let n = arr.length;
				arr[n] = {lon:c[0], lat:c[1]};
				if(flag_out_n){
					arr[n].name = f_out_n_i(i);
				}else{
					let c_n = f_out_n_c(arr_c_g[i]);
					arr[n].name = "N"+c_n[1]+" E"+c_n[0];
				}
				arr_WM[n] = from_wgs_84([toRad(c[0]), toRad(c[1])]);
			};
			let obj = {dop:{n:date_new.toISOString().slice(0,10)+"_grid_"+(interval_E % 1000 === 0 ? (interval_E / 1000).toFixed()+"km": interval_E+"m"), d:date_new, f_n_p:false, f_n_r:false, f_n_t:false, _c:input_color.value, lim_d:lim_d}, gpx:{creator:"feb7e9c4d5539ae0f1496cdb2aaee27c"}};
			if(select_save_d.selectedIndex === 1){//
				obj.rtes = [{rtepts:[], rtepts_WM:[]}];
				arr = obj.rtes[0].rtepts;
				arr_WM = obj.rtes[0].rtepts_WM;
				f_push_i = input_save_n.checked ? f_push_i_2: f_push_i_1;
			}else if(select_save_d.selectedIndex === 2){//
				obj.trks = [{trksegs:[{trkpts:[], trkpts_WM:[]}]}]
				arr = obj.trks[0].trksegs[0].trkpts;
				arr_WM = obj.trks[0].trksegs[0].trkpts_WM;
				f_push_i = input_save_n.checked ? f_push_i_2: f_push_i_1;
			}else if(select_save_d.selectedIndex === 3 || select_save_d.selectedIndex === 4 || select_save_d.selectedIndex === 5){//
				if(arr_c_g.length > 0){
					if(input_save_n.checked){
						obj.dop.f_n_p = true;
						f_push_i = f_push_i_2;
					}else f_push_i = f_push_i_1;
					arr = obj.wpts = [];// Массив.
					arr_WM = obj.wpts_WM = [];// Массив. для ускоренного доступа к точкам в Веб Меркаторе.
					for(let i = 0; i < arr_c_g.length; i++)f_push_i(i);
				}
			}
			if(select_save_d.selectedIndex === 3)flag_line = false;
			else if(select_save_d.selectedIndex === 4 || select_save_d.selectedIndex === 6){//
				obj.rtes = [{rtepts:[], rtepts_WM:[]}];
				arr = obj.rtes[0].rtepts;
				arr_WM = obj.rtes[0].rtepts_WM;
				f_push_i = f_push_i_1;
			}else if(select_save_d.selectedIndex === 5 || select_save_d.selectedIndex === 7){//
				obj.trks = [{trksegs:[{trkpts:[], trkpts_WM:[]}]}]
				arr = obj.trks[0].trksegs[0].trkpts;
				arr_WM = obj.trks[0].trksegs[0].trkpts_WM;
				f_push_i = f_push_i_1;
			...
			if(flag_line){
				let f_push = (...arr_i) => {
					for(let i of arr_i)f_push_i(i);
				};
				/// Определяем основные характеристики сетки.
				// размер.
				let l = arr_c_g.length;
				// Индекс последнего.
				let k = l - 1;
				// ширина.
				let w = w_g;
				...
				let sh = h * w - w;
				/// Построение маршрута. ///
				let i;// для всяких циклов. или переходов от одной точки к другой.
				let flag_1or2 = 1;
				let to_right = () => {//
					f_push_i(i);
					if(inner){
						while(++i % w !== sw)if((i / w) & (sw & 1) ^ (i & 1) ^ flag_1or2)f_push_i(i);
					}else i += sw;
					f_push_i(i);
				};
				let to_left = () => {//
					f_push_i(i);
					if(inner){
						while(--i % w !== 0)if((i / w) & (sw & 1) ^ (i & 1) ^ flag_1or2)f_push_i(i);
					}else i -= sw;
					f_push_i(i);
				};
				let to_top = () => {//
					f_push_i(i);
					if(inner){
						while((i += w) < sh)if((i / w) & (sw & 1) ^ (i & 1 ^ 1) ^ flag_1or2)f_push_i(i);
					}else i += sh;
					f_push_i(i);
				};
				let to_bot = () => {//
					f_push_i(i);
					if(inner){
						while((i -= w) > sw)if((i / w) & (sw & 1) ^ (i & 1 ^ 1) ^ flag_1or2)f_push_i(i);
					}else i -= sh;
					f_push_i(i);
				};
				let to_zz_left = () => {// Загзаг.
					while(true){
						if(i <= sw){
							to_top();
							if(i > sh + 1)i--;
				...
				let to_sp_bot = () => {// Спираль, делается с угла. для четных.
					f_push_i(i);
					if(h === 2)i -= w;// TEST добавлено при ошибке когда одна сторона ровна 2.
					else{
						i -= w * 2;
						if(i > sw)while(true){
							if(i % w === sw)to_left();
							else if(i % w === 0)to_right();
							else break;
							i += w;
							if(i % w === sw)to_left();
							else if(i % w === 0)to_right();
				...
				let to_sp_left = () => {// Спираль, делается с угла. для четных.
					f_push_i(i);
					if(w === 2)i--;// TEST добавлено при ошибке когда одна сторона ровна 2.
					else{
						i -= 2;
						if(i !== 0 && i !== sh)while(true){
							if(i <= sw)to_top();
							else if(i >= sh)to_bot();
							else break;
							i++;
				...
				if(rem === 0){// Определяем с остатком или без.
					if(w === 1 || h === 1){// 0. Самая простая ситуация, решается простым перебором.
						f_push_i(0);
						if(inner)for(i = inner ? 1: k; i < l; i++)f_push_i(i);
						else f_push_i(k);
					}else if((w === 2 || w === 3) && (h === 2 || h === 3)){// 1. Первое исключение, когда общая точка будет всегда, когда каждая из сторон от двух до трех.
						if(w === 3){
							f_push_i(1);
							if(h === 3){
								f_push(7, 8, 6, 0, 2, 8, 5);
								if(inner)f_push_i(4);
								f_push_i(3);
							}else f_push(4, 5, 2, 0, 3, 4);
						}else{
							f_push(2, 3, 1, 0);
							if(h === 3)f_push(4, 5, 3);
							else f_push_i(2);
						}
					}else if(w === 3){// 2. Второе исключение если одна сторона равна трем. Если недоступно внутреннее пространство, или разрывать маршрут, будет общая точка.
						flag_1or2 = h & 1;
						i = 1;
						to_top();
						if(inner){
						...
						to_zz_bot();
					}else if(h === 3){// 2. Второе исключение если одна сторона равна трем. Если недоступно внутреннее пространство, или разрывать маршрут, будет общая точка.
						flag_1or2 = sw & 1;
						i = w;
						to_right();
						if(inner){
						...
						to_zz_left();
					}else if((w & 1 & h) === 1){// 3. Третье последнее исключение, Если по высоте и ширене нечетно, то если нет внутреннего прохода то одна повторная точка.
						flag_1or2 = w & 1 ^ 1;
						i = sh - w;
						to_zz_bot();
						if(inner){
						...
						to_zz_left();
					}else if((w & 1) === 1 || h === 2){// Если не четная ширина, начинаем с ширины. Подходит и для четных.
						i = sw - 1;
						to_top();
						i = sh;
						to_sp_bot();
						...
						i = k - 2;
						to_zz_left();
					}else{// if((h & 1) === 1) // Если не четная высота, начинаем с высоты. Подходит и для четных.
						i = sh - w;
						to_right();
						i = sw;
						to_sp_left();
					...
					let rw = w - rem;// Оставшееся длина.
					let rk = k - rem;// Угол.
					let rh = rk + 1;// Угол.
					let to_top_r = () => {//
						f_push_i(i);
						if(inner){
							while((i += w) < sh + w)if((i / w) & (sw & 1) ^ (i & 1 ^ 1) ^ flag_1or2)f_push_i(i);
						}else i += rh;
						f_push_i(i);
					};
					let to_bot_r = () => {//
					...
					if(h === 1){// Самый простой вариант если высота = 1.
						if(rem === 1){
							f_push_i(w);
							i = 0;
						}else{
						...
						for(; i < w; i++)f_push_i(i);
					}else if(rem === 1){// Исключение если доп равен 1.
						if(w > 2){
							if((w & 1) === 1)i = sw - 1;// Если не четная то начинаем с низу
							else i = rk - 1;// Иначе сверху.
							to_zz_left();
						}
						f_push(sh, rk, sw, 0, rh);
						if(h > 2){
							i = sh - w;
							to_zz_bot();
						}
					}else if((rem & 1) === 1){// Если доп. нечетное.
						flag_1or2 = sw & 1;// делаем в нижнем правом углу свободное место для возврата.
						i = rh + 1;
						while(true){
							to_bot_r();
							i++;
						...
						f_push(rh, 0, sw, rk);
						if((rw & 1) === 1 && rw > 1){// Если оставшееся длина четная. (rw & 1) === 1
							i = sh + rem
							while(true){
								to_bot();
								i++;
						...
						if(inner){
							i = rk - rw;
							if((i / w) & (sw & 1) ^ (i & 1 ^ 1) ^ flag_1or2)i--;
							if(i > sh)to_left();
						}else f_push_i(i = sh);
						i -= w;
						to_zz_bot();
						if((rw & 1) === 0){// Если оставшееся длина не четная.
							f_push_i(i = sw + sw);
							f_push_i(i -= w);
							if(inner){
								i += w * 3;
								to_top();
							}else f_push_i(i = rk - 1);
							while(true){
								i--;
								if(i < sh + rem)break;
								to_bot();
					...
					}else{// Если доп. четное.
						if(rw > 1){
							if((rw & 1) === 1)i = sw - 1;// Меняет то от куда начинаем с верху или с низу, если оставшееся длина не четная то сверху.
							else i = rk - 1;
							while(true){
								if(i < sw)to_top();
								else to_bot();
			...
			if(select_save_d.selectedIndex === 1){//
				if(arr.length > 0 && input_save_n.checked)obj.dop.f_n_r = true;
			}else if(select_save_d.selectedIndex === 2){//
				if(arr.length > 0 && input_save_n.checked)obj.dop.f_n_t = true;
			}else if(select_save_d.selectedIndex === 5){// Только для типа 5, на всякий случай, чтоб подходил кривому Гармину.
				let time = date_new.getTime();
				obj.gpx.metadata = {time:time};
				if(obj.wpts.length > 0){
					let minlat = obj.wpts[0].lat;
					let minlon = obj.wpts[0].lon;
					let maxlat = obj.wpts[0].lat;
					let maxlon = obj.wpts[0].lon;
					obj.wpts[0].time = time;
					for(let i = 1; i < obj.wpts.length; i++)obj.wpts[i].time = time;
					if(arr.length > 0){
						for(let i = 0; i < arr.length; i++){
							if(arr[i].lon > maxlon)maxlon = arr[i].lon;
							else if(arr[i].lon < minlon)minlon = arr[i].lon;
							if(arr[i].lat > maxlat)maxlat = arr[i].lat;
							else if(arr[i].lat < minlat)minlat = arr[i].lat;
							arr[i].time = time;
						}
					}
					obj.gpx.metadata.bounds = {minlat:minlat, minlon:minlon, maxlat:maxlat, maxlon:maxlon};
					obj.dop.st = "yes";
					obj.trks[0].name = "line";
					obj.trks[0].desc = "";
				}
			}
		...
		// Сохраняем как трек gpx.
		b_save_track.onclick = () => {
			m_tracks.f_save_gpx(f_in_gpx());
		};
		// Импортируем в менеджер треков.
		b_in_track.onclick = () => {
			m_tracks.f_track_enter_g(f_in_gpx());
		};
		...
		/// EXTRA
		this.f_load = (ctx, ctx_w, ctx_h, res, coor_top_left, coor_bot_right, coef_t) => {// Функция для прописовки сетки при сохранении.
			if(input_on.checked){
				// Создаем сассив с координатами пикселей в картинке сохранения.
				let arr_c = arr_c_g.map(coor => {
					let c = map.map.getCoordinateFromPixel(coor);
					c[0] = (c[0] - coor_top_left[0]) / res;
					c[1] = (coor_top_left[1] - c[1]) / res;
					return c;
				});
				...
				for(let i = 0; i < arr_c.length; i++){
					if(i % w_g === 0)ctx.moveTo(arr_c[i][0], arr_c[i][1]);
					else ctx.lineTo(arr_c[i][0], arr_c[i][1]);
				}
				for(let i = 0; i < w_g; i++){
					for(let j = i; j < arr_c.length; j += w_g){
						if(j === i)ctx.moveTo(arr_c[j][0], arr_c[j][1]);
						else ctx.lineTo(arr_c[j][0], arr_c[j][1]);
					}
				}
				ctx.stroke();
				/// Рисуем нумерацию сетки если есть.
				if(input_on_n.checked){//out_n();
					if(arr_c.length < 2)return;
					let l = ((arr_c[1][0] - arr_c[0][0]) ** 2 + (arr_c[1][1] - arr_c[0][1]) ** 2) ** .5;
					l *= n_size * coef_t;
					if(l < 15)return;// Если слишком мелкий текст то невыводим.
					if(flag_out_n){// Вариант обычеой нумерации
						remainder_n_w = arr_c.length % w_g;// Определяем остаток если сработал лимит.
						h_g = (arr_c.length - remainder_n_w) / w_g;// определяем высоту
						// опрнднляем количество знаков в нумерации.
						let l_t = f_out_n_i(f_n_last_i(arr_c.length)).length;
						// Расчитываем шрифт и другие величины ширены и высоты текста нумерации.
						let height_t = l / 3 | 0;
						height_t = height_t / ((l_t < 4 ? 4: l_t) / 4) | 0;
						ctx.font = height_t+"px monospace";
						...
						ctx.fillStyle = n_color;
						for(let i = 0; i < arr_c.length; i++){
							if(arr_c[i][0] < 0 || arr_c[i][0] > ctx_w)continue;
							if(arr_c[i][1] < height_t_2 || arr_c[i][1] > ctx_h + height_t_2)continue;
							ctx.fillRect(arr_c[i][0] - width_t_2, arr_c[i][1] - height_t, width_t, height_t);
						}
						// Рисуем текст.
						ctx.fillStyle = n_color_t;
						ctx.textAlign = "center";
						for(let i = 0; i < arr_c.length; i++){
							if(arr_c[i][0] < 0 || arr_c[i][0] > ctx_w)continue;
							if(arr_c[i][1] < height_t_2 || arr_c[i][1] > ctx_h + height_t_2)continue;
							ctx.fillText(f_out_n_i(i), arr_c[i][0], arr_c[i][1]);
						}
					}else{// Вариант координатами нумерации
						//f_out_n_c(arr_c_g[0]);
						if(input_after.valueAsNumber === 0)return;// Если длина текста ноль то невыводим.
						...
						for(let i = 0; i < arr_c.length; i++){
							if(arr_c[i][0] < 0 || arr_c[i][0] > ctx_w)continue;
							if(arr_c[i][1] < height_t_2 || arr_c[i][1] > ctx_h + height_t_2)continue;
							ctx.fillRect(arr_c[i][0] - width_t_2, arr_c[i][1] - height_t, width_t, height_2t);
						}
						// Рисуем текст. Сначала первую координату отдельно.
						ctx.fillStyle = n_color_t;
						ctx.textAlign = "end";
						if(arr_c[0][0] >= 0 && arr_c[0][0] <= ctx_w && arr_c[0][1] >= height_t_2 && arr_c[0][1] <= ctx_h + height_t_2){
							ctx.fillText(c[1], arr_c[0][0] + width_t_2, arr_c[0][1]);
							ctx.fillText(c[0], arr_c[0][0] + width_t_2, arr_c[0][1] + height_t);
						}
						for(let i = 1; i < arr_c.length; i++){
							if(arr_c[i][0] < 0 || arr_c[i][0] > ctx_w)continue;
							if(arr_c[i][1] < height_t_2 || arr_c[i][1] > ctx_h + height_t_2)continue;
							c = f_out_n_c(arr_c_g[i]);
							ctx.fillText(c[1], arr_c[i][0] + width_t_2, arr_c[i][1]);
							ctx.fillText(c[0], arr_c[i][0] + width_t_2, arr_c[i][1] + height_t);
						}
					}
				}
