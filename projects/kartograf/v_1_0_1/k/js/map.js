function Map(){
	...
	let f_source_opt = function(id, arr){
		...
		if(arr[id].tileLoadFunction !== undefined)options.tileLoadFunction = arr[id].tileLoadFunction;
		if(arr[id].projection !== undefined)options.projection = arr[id].projection;
		//options.crossOrigin = 'anonymous';// Необходимо для сохранения карт в канвасе. CORS.
		return options;
	};
	/*
	let f_source_opt_not_anonymous = function(id, arr){// ВРЕМЕННО потом доделать. Как вариант поискать зеркало.
		let url = arr[id].url == undefined ? arr[0].url: arr[id].url;
		let options = {
			maxZoom:arr[id].max_z == undefined ? arr[0].max_z: arr[id].max_z,
			minZoom:arr[id].min_z == undefined ? arr[0].min_z: arr[id].min_z,
		...
		//options.crossOrigin = 'anonymous';// Необходимо для сохранения карт в канвасе. CORS.// ВРЕМЕННО потом доделать.
		return options;
	};//*/
	...
	for(let i = 1; i < g_srtm.length; i++){
		...
		input.onchange = function(){
			if(input.checked){
				...
				this.layers_srtm.push(layer);
			}else{
				layer.setMap(null);
				this.layers_srtm.splice(this.layers_srtm.indexOf(layer), 1);
			}
		}.bind(this);
	};
	...
	let source_FIRMS = new ol.source.ImageWMS({
		...
		//crossOrigin:'anonymous',// Необходимо для сохранения карт в канвасе. CORS.
		params:{
	...
	//this.get_layer_FIRMS = () => layer_FIRMS;
