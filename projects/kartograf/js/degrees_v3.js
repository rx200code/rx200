function Degrees(){
	...
	let top_left_text = create_bord();// Рамка/подложка для текста.
	degrees_text.appendChild(top_left_text);
	
	let scale_text = 1;// Коэфицент масштаба текста.
	let size_t = 18 * scale_text;// Размер текста.
	// Создает текст который в углах карты.
	let create_text_a = (x, y, align) => {
		let elm = create_text();
		elm.setAttributeNS(null,'font-size', size_t);
		elm.setAttributeNS(null,'x', x);
		elm.setAttributeNS(null,'y', y);
		elm.setAttributeNS(null, "text-anchor", align);//Центрируем текст по оси "x".
		//text.setAttributeNS(null, "pointer-events", "all");// Стоит ли делать доступным для событий мыши текст, чтоб можно было выделять и копировапть.
		return elm;
	};
	// Настраиваем позицию текстов.
	let left_x = 3 * scale_text;
	let right_x = width_map - 1 * scale_text;
	let top_y = 14 * scale_text;
	let bot_y_1 = height_map - 17 * scale_text;
	let bot_y_2 = height_map - 3 * scale_text;
	// Иоздаем тексты для углов карты.
	let top_left_text_F = create_text_a(left_x, top_y, "start");
	let top_left_text_L = create_text_a(left_x, top_y * 2, "start");
	let top_right_text_F = create_text_a(right_x, top_y, "end");
	let top_right_text_L = create_text_a(right_x, top_y * 2, "end");
	let bot_left_text_F = create_text_a(left_x, bot_y_1, "start");
	let bot_left_text_L = create_text_a(left_x, bot_y_2, "start");
	let bot_right_text_F = create_text_a(right_x, bot_y_1, "end");
	let bot_right_text_L = create_text_a(right_x, bot_y_2, "end");
	degrees_text.appendChild(top_left_text_F);
	degrees_text.appendChild(top_left_text_L);
	degrees_text.appendChild(top_right_text_F);
	degrees_text.appendChild(top_right_text_L);
	degrees_text.appendChild(bot_left_text_F);
	degrees_text.appendChild(bot_left_text_L);
	degrees_text.appendChild(bot_right_text_F);
	degrees_text.appendChild(bot_right_text_L);
	// Функция настройки подложки текста, вызывается отдельно при смене формата текста, и в начале, чтоб не вызываеть её каждый раз при выводе текста.
	let format_d = document.getElementById("format_d").selectedIndex;
	let f_format_d = () => {
		format_d = document.getElementById("format_d").selectedIndex;
		let width = 123 * scale_text;
		if(format_d == 4)width = 144 * scale_text;
		else if(format_d == 5)width = 165 * scale_text;
		else if(format_d == 6 || format_d == 7)width = 186 * scale_text;
		else if(format_d == 8 || format_d == 15 || format_d == 16)width = 105 * scale_text;
		else if(format_d == 10)width = 158 * scale_text;
		else if(format_d == 11 || format_d == 12 || format_d == 13)width = 135 * scale_text;
		let height = 32 * scale_text;
		let bord = "h"+width+"v"+height+"h-"+width+"z";
		top_left_text.setAttributeNS(null, "d", "M0,0"+bord+"M"+(width_map - width)+",0"+bord+"M"+(width_map - width)+","+(height_map - height)+bord+"M0,"+(height_map - height)+bord);
	};
	// Вункция подготовки вывода текста в соответствии с форматом вывода.
	...
	let f_to_xy = () => {
		...
		if(document.getElementById("format_d").selectedIndex < 8){
			document.getElementById("format_d").selectedIndex = 8;
			f_format_d();
		}
	}
	let f_to_fl = () => {
		...
		if(document.getElementById("format_d").selectedIndex > 7){
			document.getElementById("format_d").selectedIndex = 1;
			f_format_d();
		}
	}
	...
	let f_sys_c = () => {
		let sys_c = document.getElementById("sys_c").selectedIndex;
		switch(sys_c){
			case 1:
				f_coor_transform = to_wgs_84_deg;
				f_to_fl();
				break;
			case 2:
				f_coor_transform = (coor) => coor;
				f_to_xy();
				break;
			case 3:
				f_coor_transform = (coor) => coor;
				...
				break;
			case 5:
				f_coor_transform = to_4284_proj4;
				f_to_fl();
				break;
			case 6:
				f_coor_transform = to_GK_proj4;
				f_to_xy();
				break;
			case 7:
				f_coor_transform = to_wgs_84_proj4;
				f_to_fl();
				break;
			case 8:
				f_coor_transform = to_UTM_proj4;
				f_to_xy();
				break;
			default:
				f_coor_transform = to_wgs_84_deg;
	...
	let f_out_angles_text = () => {
		// 1. формируем 4 угловые координаты
		// coor_top_left // Есть.
		let coor_bot_right = map.map.getCoordinateFromPixel([width_map,height_map]);
		let coor_top_right = [coor_bot_right[0], coor_top_left[1]];
		let coor_bot_left = [coor_top_left[0], coor_bot_right[1]];
		// 2. Переводим их в нужную систему/проекцию координат
		...
		// 3. Переводим в нужный формат, отображения координат.
		// 4. Отображаем координаты.
		let z = c_top_left_out.length == 3;
		top_left_text_F.textContent = f_number_in_format_d(c_top_left_out[1], false)+(z ? " "+c_top_left_out[2]: "");
		top_left_text_L.textContent = f_number_in_format_d(c_top_left_out[0], true);
		top_right_text_F.textContent = (z ? c_top_right_out[2]: "")+f_number_in_format_d(c_top_right_out[1], false);
		top_right_text_L.textContent = f_number_in_format_d(c_top_right_out[0], true);
		bot_left_text_F.textContent = f_number_in_format_d(c_bot_left_out[1], false)+(z ? " "+c_bot_left_out[2]: "");
		bot_left_text_L.textContent = f_number_in_format_d(c_bot_left_out[0], true);
		bot_right_text_F.textContent =(z ? c_bot_right_out[2]: "")+ f_number_in_format_d(c_bot_right_out[1], false);
		...
		/*
		let coor_bot_right = map.map.getCoordinateFromPixel([width_map,height_map]);
		let coor_top_left_F = f_number_in_format_d(coor_top_left[1], false);
		let coor_top_left_L = f_number_in_format_d(coor_top_left[0], true);
		let coor_bot_right_F = f_number_in_format_d(coor_bot_right[1], false);
		let coor_bot_right_L = f_number_in_format_d(coor_bot_right[0], true);
		//*/
		
		/*
		let coor_bot_right = map.map.getCoordinateFromPixel([width_map,height_map]);
		let coor_top_left_F = coor_top_left[1];
		let coor_top_left_L = coor_top_left[0];
		let coor_bot_right_F = coor_bot_right[1];
		let coor_bot_right_L = coor_bot_right[0];
		//*/
		/*
		top_left_text_F.textContent = coor_top_left_F;
		top_left_text_L.textContent = coor_top_left_L;
		top_right_text_F.textContent = coor_top_left_F;
		top_right_text_L.textContent = coor_bot_right_L;
		bot_left_text_F.textContent = coor_bot_right_F;
		bot_left_text_L.textContent = coor_top_left_L;
		bot_right_text_F.textContent = coor_bot_right_F;
		bot_right_text_L.textContent = coor_bot_right_L;
		//*/
	};
	...
	menu.onchange = function(e){
		...
		if(e.target.id == "markings_corners"){
			if(document.getElementById("markings_corners").checked){
				f_sys_c();
				f_format_d();
				this.svg.appendChild(degrees_text);
				document.querySelector(".ol-attribution").style.display = "none";
				f_out_angles();
				map.map.on("postrender", f_out_angles);// пока самая надежная "postrender"
			}else{
				if(this.svg.contains(degrees_text)){
					this.svg.removeChild(degrees_text);
					map.map.un("postrender", f_out_angles);
					document.querySelector(".ol-attribution").style.display = "inline-block";
				}
			}
		}
		if(document.getElementById("markings_corners").checked){
			if(e.target.id == "format_d"){
				f_format_d();
			};
			if(e.target.id == "sys_c"){
				f_sys_c();
			};
			f_out_angles_text();
		}
	}.bind(this);
	map.map.once('postrender', function(){menu.onchange({target:{id:"markings_corners"}});});
