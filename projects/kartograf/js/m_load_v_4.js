function M_load(){
	...
	g_zoom.insertAdjacentHTML('beforeend', "<br><br>Размеры сохраняемой карты в пикселях не рекомендуется превышать 4000x4000<br>width : ");
	...
	let f_wh_p = int => {
		let w_c = coor_b_r[0] - coor_t_l[0];// Ширена в ВМ.
		let h_c = coor_t_l[1] - coor_b_r[1];// Высота в ВМ.
		let res_true = width_WM / (256 * (2 ** int));// Истинное разрешение нужно если разные слои не совпадают по мах зумму с нужным зумом.
		out_width.textContent = w_c / res_true;// Ширена в пикселях. // Всей картинки.
		out_height.textContent = h_c / res_true;// Высота в пикселях. // Всей картинки.
	};
	b_zoom.oninput = () => {
