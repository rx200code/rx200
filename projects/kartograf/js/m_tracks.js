function Manager_tracks(){
	...
	to_wpt = (obj, tag = "wpt") => {
		...
		let text = '<'+tag+' lat="'+parseFloat(obj.lat.toFixed(lim_d))+'" lon="'+parseFloat(n.toFixed(lim_d))+'">';
		if(obj.ele !== undefined)text += '<ele>'+obj.ele+'</ele>';
		if(obj.time !== undefined){
			tr_date.setTime(obj.time);
			text += '<time>'+tr_date.toISOString()+'</time>';
		}
		if(obj.magvar !== undefined)text += '<magvar>'+obj.magvar+'</magvar>';
		if(obj.geoidheight !== undefined)text += '<geoidheight>'+obj.geoidheight+'</geoidheight>';
		if(obj.name !== undefined)text += '<name>'+obj.name+'</name>';
		if(obj.cmt !== undefined)text += '<cmt>'+obj.cmt+'</cmt>';
		if(obj.desc !== undefined)text += '<desc>'+obj.desc+'</desc>';
		if(obj.src !== undefined)text += '<src>'+obj.src+'</src>';
		if(obj.links !== undefined)for(let i = 0; i < obj.links.length; i++)text += to_link(obj.links[i]);
		if(obj.sym !== undefined)text += '<sym>'+obj.sym+'</sym>';
		if(obj.type !== undefined)text += '<type>'+obj.type+'</type>';
		if(obj.fix !== undefined)text += '<fix>'+obj.fix+'</fix>';
		if(obj.sat !== undefined)text += '<sat>'+obj.sat+'</sat>';
		if(obj.hdop !== undefined)text += '<hdop>'+obj.hdop+'</hdop>';
		if(obj.vdop !== undefined)text += '<vdop>'+obj.vdop+'</vdop>';
		if(obj.pdop !== undefined)text += '<pdop>'+obj.pdop+'</pdop>';
		if(obj.ageofdgpsdata !== undefined)text += '<ageofdgpsdata>'+obj.ageofdgpsdata+'</ageofdgpsdata>';
		if(obj.dgpsid !== undefined)text += '<dgpsid>'+obj.dgpsid+'</dgpsid>';
		if(obj.extensions !== undefined)text += to_extensions(obj.extensions);
		text += '</'+tag+'>';
		return text;
	};
	to_rte = obj => {
		...
		if(obj.src !== undefined)text += '<src>'+obj.src+'</src>';
		if(obj.links !== undefined)for(let i = 0; i < obj.links.length; i++)text += to_link(obj.links[i]);
		if(obj.number !== undefined)text += '<number>'+obj.number+'</number>';
		if(obj.type !== undefined)text += '<type>'+obj.type+'</type>';
		if(obj.extensions !== undefined)text += to_extensions(obj.extensions);
	...
	to_gpx_file = obj => {
		...
		// Декларация.
		let text = '<?xml version="'+(obj.dop.v !== undefined ? obj.dop.v: "1.0")+'" encoding="UTF-8"'+(obj.dop.st !== undefined ? ' standalone="'+obj.dop.st+'"?>': '?>');
		// Корневой gpx тег.
