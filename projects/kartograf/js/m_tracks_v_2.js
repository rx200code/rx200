function Manager_tracks(){
	...
	let input_color = document.getElementById("tr_color");// Устанавливает цвет.
	//track_list
	...
	let tr_obj;// Для временного копирования ссылки на объект трека.
	input_color.onchange = () => {
		//alert(input_color.clientTop);
		//alert(input_color.offsetTop);
		tr_obj.dop.c = input_color.value;
	};
	...
	// Функция добавляет треки, в соответствии с правилом сортировки, в конце загрузки всех файлов.
	end_l = () => {
		count_l++;
		if(count_l >= input_file_gpx.files.length){
			count_l = 0;
			f_error_out("Итог", "Загружено "+arr_pre_tr.length+" из "+input_file_gpx.files.length, arr_pre_tr.length !== input_file_gpx.files.length);
			for(let i = 0; i < arr_pre_tr.length; i++){
				
				let j = 0;
				while(j < arr_tr.length){
					// Сортировка по имени.
					if(arr_tr[j].dop.n.localeCompare(arr_pre_tr[i].dop.n) >= 0)break;
					j++;
				}
				arr_tr.splice(j, 0, arr_pre_tr[i]);
			}
	...
	let p_point = createElementNS("path", ["fill", "none"], ["stroke-width", 5], ["stroke-opacity", 1], ["stroke", "#00f"], ["stroke-linecap", "round"]);
	svg.appendChild(p_point);
	...
	let f_tr_colof = (obj, e) => {
		//input_color.offsetTop = 500;
		
		//input_color.scrollLeft = 200;
		
		//obj.menu.append(input_color);
		...
		//alert(input_color.scrollLeft);
	};
	let f_track_enter = (obj) => {
		// Создаем контейнер.
		...
		obj.menu.onclick = e => {
			if(e.target.className === "track"){
				flag = !flag;
				obj.menu.style.backgroundColor = flag ? "#ffe": "#eff";
			}
		};
		// Имя файла.
		...
		b_color.innerHTML = " &nbsp;";
		b_color.onclick = e => {f_tr_colof(obj, e);};
		
		Object.defineProperty(obj.dop, 'c', {get(){return this._c}, set(c){this._c = c; b_color.style.backgroundColor = c;}});
		
		obj.menu.append(b_color);
		
		//
		obj.menu.insertAdjacentHTML('beforeend', "<br>");
		let input_all_view = createElement("input", ["type", "checkbox"], ["checked", true]);
		obj.menu.append(input_all_view);
		//
