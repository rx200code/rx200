function Degrees(){
	...
	let grid = () => {
		...
		let input_n_t_1 = createElement("input", ["type", "text"], ["size", 5], ["value", "A"]);
		span_n_a.append(input_n_t_1);
		span_n_a.insertAdjacentText('beforeend', " ");
		
		let input_n_2 = createElement("input", ["type", "checkbox"], ["checked", true]);
		span_n_a.append(input_n_2);
		
		let input_n_t_2 = createElement("input", ["type", "text"], ["size", 5], ["value", "1"]);
		span_n_a.append(input_n_t_2);
		...
		let type_n_1, type_n_2, offset_n_1, offset_n_2;
		let out_n_d = "";
		type_n_1 = 2;
		type_n_2 = 1;
		offset_n_1 = 1;
		...
		let f_out_n_i_SE = i => number_in_text(w_g - i % w_g - 1 + offset_n_1, type_n_1)+out_n_d+number_in_text((i / w_g | 0) + offset_n_2, type_n_2);
		let f_out_n_i_SE_v = i => {
			let h = i % w_g;
			return number_in_text(arr_c_g.length - (h_g - (i / w_g | 0) + h_g * h + (h < remainder_n_w ? h + 1: remainder_n_w)) + offset_n_2, type_n_2);
		};
		let f_out_n_i_SE_g = i => {
			if(i + 1 > w_g * h_g)return number_in_text(arr_c_g.length - (remainder_n_w - (arr_c_g.length - i) + 1) + offset_n_1, type_n_1);
			else return number_in_text(arr_c_g.length - (arr_c_g.length - i + w_g - ((w_g - (i % w_g)) * 2) + 1) + offset_n_1, type_n_1);
		};
		// Верхний правый угол. N E
		let f_out_n_i_NE = i => number_in_text(w_g - i % w_g - 1 + offset_n_1, type_n_1)+out_n_d+number_in_text((remainder_n_w === 0 ? h_g - 1: h_g) - (i / w_g | 0) + offset_n_2, type_n_2);
		let f_out_n_i_NE_v = i => {
			let h = i % w_g;
			return number_in_text(arr_c_g.length - ((i / w_g + 1 | 0) + h_g * h + (h > remainder_n_w ? remainder_n_w: h)) + offset_n_2, type_n_2);
		};
		let f_out_n_i_NE_g = i => number_in_text(arr_c_g.length - (i + 1) + offset_n_1, type_n_1);
		// Верхний левый угол. NW
		let f_out_n_i_NW = i => number_in_text(i % w_g + offset_n_1, type_n_1)+out_n_d+number_in_text((remainder_n_w === 0 ? h_g - 1: h_g) - (i / w_g | 0) + offset_n_2, type_n_2);
		let f_out_n_i_NW_v = i => {
			let h = i % w_g;
			return number_in_text(h_g - (i / w_g | 0) + h_g * h + (h < remainder_n_w ? h: remainder_n_w - 1) + offset_n_2, type_n_2);
		};
		let f_out_n_i_NW_g = i => {
			if(i + 1 > w_g * h_g)return number_in_text(remainder_n_w - (arr_c_g.length - i) + offset_n_1, type_n_1);
			else return number_in_text(arr_c_g.length - i + w_g - (w_g - (i % w_g)) * 2 + offset_n_1, type_n_1);
		};
		// Нижний левый угол. S W
		let f_out_n_i_SW = i => number_in_text(i % w_g + offset_n_1, type_n_1)+out_n_d+number_in_text((i / w_g | 0) + offset_n_2, type_n_2);
		let f_out_n_i_SW_v = i => {
			let h = i % w_g;
			return number_in_text((i / w_g | 0) + h_g * h + (h > remainder_n_w ? remainder_n_w: h) + offset_n_2, type_n_2);
		};
		let f_out_n_i_SW_g = i => number_in_text(i + offset_n_1, type_n_1);
		...
		let out_n = () => {
			...
			for(let i = 0; i < arr_c_g.length; i++){
				...
				if(arr_c_g[i][1] < height_t / 2 || arr_c_g[i][1] > height_map + height_t / 2)continue;
				ctx.fillRect(arr_c_g[i][0] - width_t_2, arr_c_g[i][1] - height_t, width_t, height_t);
				//let ttt = arr_c_g[i][0]+"";
				//ctx.fillText('█'.repeat(ttt.length), arr_c_g[i][0], arr_c_g[i][1]);
			}//*/
			//ctx.strokeStyle = "#fff";
			ctx.fillStyle = "#000";
