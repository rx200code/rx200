function Degrees(){
	...
	let grid = () => {
		...
		//span_n_a.style.display = "none";// Временно
		span.append(span_n_a);
		
		// Нумерация координатами.
		...
		let input_before = createElement("input", ["type", "range"], ["class", "r_color"], ["min", "0"], ["max", "10"], ["step", "1"], ["value", 0]);
		span_n_c.append(input_before);
		span_n_c.insertAdjacentText('beforeend', ",");
		
		let input_after = createElement("input", ["type", "range"], ["class", "r_color"], ["min", "0"], ["max", "10"], ["step", "1"], ["value", 10]);
		span_n_c.append(input_after);
		
		let label_auto_n_c = createElement("label");
			label_auto_n_c.insertAdjacentText('beforeend', " auto:");
		let input_auto_n_c = createElement("input", ["type", "checkbox"], ["checked", true]);
		label_auto_n_c.append(input_auto_n_c);
		span_n_c.append(label_auto_n_c);
		
		
		span_n_c.style.display = "none";// Временно
		...
		let layer_n = createElement("canvas", ["width", width_map], ["height", height_map], ["style", "position: absolute; top:0px; left:0px; pointer-events: none;"]);// Временно тут.
		...
		// Выводим, добавляем, удаляем, сетку на карту.
		...
		let _sc_n;
		let f_out_n_c_test = c => {
			//let coor = crs.sc[0].f_to(c[2]).map(toDeg);
			//let coor = crs.sc[0].f_to(map.map.getCoordinateFromPixel(c)).map(toDeg).map((n) => n.toFixed(2));
			//let coor = crs.sc[1].f_to([...map.map.getCoordinateFromPixel(c), ...z_s]);
			//let coor = crs.sc[1].f_to(map.map.getCoordinateFromPixel(c));
			let coor = _sc.f_to([...map.map.getCoordinateFromPixel(c), ...z_s]);
			//coor[0] |= 0;
			coor[0] /= 1000;
			//coor[0] = Math.round(coor[0]);
			
			coor[0] %= 100;
			coor[0] = coor[0].toFixed(1);
			
			//coor[1] |= 0;
			coor[1] /= 1000;
			//coor[1] = Math.round(coor[1]);
			
			coor[1] %= 100;
			coor[1] = coor[1].toFixed(1);
			return coor;
		};
		...
		let n_c_mul = 1;
		let f_out_n_c_deg = c => {
			// неучтено если числа разной длены с начала числа.
			let coor = _sc_n.f_to(map.map.getCoordinateFromPixel(c)).map(toDeg).map(n => n.toString().slice(input_before.value, input_before.value * 1 + input_after.value * 1));
			return coor;
		};
		let f_out_n_c_m_v_1 = c => {
			let coor = _sc_n.f_to([...map.map.getCoordinateFromPixel(c), ...z_s]);
			
			
			let minus_0 = coor[0] < 0 ? "-": "";
			let minus_1 = coor[1] < 0 ? "-": "";
			
			//coor[0] = Math.abs(coor[0]).toFixed();
			//coor[1] = Math.abs(coor[1]).toFixed();
			coor[0] = coor[0].toFixed();
			coor[1] = coor[1].toFixed();
			
			if(coor[0].length > coor[1].length)coor[1] = " ".repeat(coor[0].length - coor[1].length)+coor[1];
			else coor[0] = " ".repeat(coor[1].length - coor[0].length)+coor[0];
			...
			//let add = Math.abs((Math.log10(Math.abs(coor[0])) | 0) - (Math.log10(Math.abs(coor[1])) | 0));
			...
			coor[0] %= 10 ** (14 - input_before.value);
			coor[1] %= 10 ** (14 - input_before.value);
			
			coor[0] /= 1000;
			coor[1] /= 1000;
			//*/
			
			//coor[0] /= 10 ** (input_after.value - (14 - input_before.value));
			//coor[1] /= 10 ** (input_after.value - (14 - input_before.value));
			/*
			if(Math.abs(coor[0]) > Math.abs(coor[1])){
				coor[1] = " ".repeat(add)+coor[1];
				coor[0] += "";
			}else{
				coor[0] = " ".repeat(add)+coor[0];
				coor[1] += "";
			}//*/
			//alert(add);
			/*
			let lim = coor[0].length - input_before.value;
			let start = (input_before.value * -1 - input_after.value);
			coor[0] = minus_0+coor[0].slice(start, lim);
			lim = coor[1].length - input_before.value;
			start = (input_before.value * -1 - input_after.value);
			coor[1] = minus_1+coor[1].slice(start, lim);
			//*/
			//coor[0] = minus_0+coor[0].slice(input_after.value * -1 - input_before.value, input_after.value * -1);
			//coor[1] = minus_1+coor[1].slice(input_after.value * -1 - input_before.value, input_after.value * -1);
			//coor[0] = minus_0+coor[0].toString().slice(input_before.value, input_before.value * 1 + input_after.value * 1);
			//coor[1] = minus_1+coor[1].toString().slice(input_before.value, input_before.value * 1 + input_after.value * 1);
			coor[0] = coor[0].toString().slice(input_before.value, input_before.value * 1 + input_after.value * 1);
			coor[1] = coor[1].toString().slice(input_before.value, input_before.value * 1 + input_after.value * 1);
			return coor;
		};
		...
		let f_out_n_c_m = c => {
			...
			let n_start_mod = z_max + 1 - input_before.value
			...
			let n_end_mod = n_start_mod - input_after.value;
			if(n_end_mod < 0){
				// Округляем.
				// 7.2 максимум 14 знаков.
		...
		let out_n = () => {
			...
			if(flag_out_n){
				remainder_n_w = arr_c_g.length % w_g;
				h_g = (arr_c_g.length - remainder_n_w) / w_g;
				
				//document.getElementById("test_grid").textContent = l;
				let height_t = l / 3 | 0;
				...
				let tt = (arr_c_g.length - 1)+"";//let tt = arr_c_g.length+"";
				let width_t = width_t_0 * tt.length;
				...
				let n_input = input_after.value * 1 + 1;
				if(n_input < 5)n_input = 5;
				let height_t = l / 3;
				height_t = height_t / (n_input / 4) | 0;
				...
				for(let i = 0; i < arr_c_g.length; i++){
					if(arr_c_g[i][0] < 0 || arr_c_g[i][0] > width_map)continue;
					if(arr_c_g[i][1] < height_t_2 || arr_c_g[i][1] > height_map + height_t_2)continue;
					let c = f_out_n_c(arr_c_g[i]);
					//ctx.fillText(c[1], arr_c_g[i][0], arr_c_g[i][1]);
					//ctx.fillText(c[0], arr_c_g[i][0], arr_c_g[i][1] + height_t);
		...
		input_before.setAttribute("min", 0);
		input_before.setAttribute("max", 15);
		input_before.setAttribute("value", 0);
		
		input_after.setAttribute("min", 0);
		input_after.setAttribute("max", 10);
		input_after.setAttribute("value", 4);
		
		
		input_before.oninput = () => {// временно тут.
			out_n();
		};
		input_after.oninput = () => {// временно тут.
		...
		select_n_t.onchange = () => {// временно тут.
		...
		let f_out_selector = () => {
			if(select_a_g.selectedIndex < 5){
				...
				flag_out_n = false;
				if(select_a_g.selectedIndex === 6){
					_sc_n = _sc;
					f_out_n_format(true);
					f_out_n_c = f_out_n_c_m;
				}else{
					_sc_n = crs.sc[select_a_g.selectedIndex - 7];
					f_out_n_format(_sc_n.metre);
					if(_sc_n.metre){
						f_out_n_c = f_out_n_c_m;
					}else{
						//f_out_n_c = f_out_n_c_deg;
						f_out_n_c = f_out_n_c_m;
					}
				}
		...
		input_on_n.onchange = () => {
			...
			}else if(document.body.contains(layer_n))document.body.removeChild(layer_n);
		};
