...
rx.map = (id, center) => {
	...
	let c_left_top = [x_min + (width_WM / 2), y_max + (width_WM / 2)];// в системе координат тайлов // всегда должны быть актуальными, относительно их и будет считаться все остальное.
	...
	// Настройки изначального масштаба.
	let deltaY = 1;
	let step = 0.1;
	step /= 2;
	elm_map.onwheel = event => {
		...
		deltaY += step * Math.sign(event.deltaY);//Для совместимости браузеров, от дельты берем только знак.
		let flag_offset = false;
		if(deltaY > 1){
			deltaY -= 0.5;
			zoom++;
		...
		res = width_WM / (size_t * (2 ** (zoom - (1 - deltaY))));// Разрешение.
		...
		//out("x = "+cursor_offsetX+"<br>y = "+cursor_offsetY);
		deltaX *= deltaY;
		//let deltaX = 1 + step * Math.sign(event.deltaY);
		
		
		ex[0] += ((cursor_offsetX) - ((cursor_offsetX) * deltaX)) * res;
		ex[3] += ((cursor_offsetY) - ((cursor_offsetY) * deltaX)) * res;
		
		imgs.forEach(img => {
			//let flag_offset = false;
			//let left = img.offsetLeft + move_x;
			//let left = img.ctx_x + ((event.clientX - img.ctx_x) - ((event.clientX - img.ctx_x) * deltaX));//move_x;
			let left = img.ctx_x + ((cursor_offsetX - img.ctx_x) - ((cursor_offsetX - img.ctx_x) * deltaX));//move_x;
			/*
			if(left > limit_left){
				flag_offset = true;
				img.title_x -= title_w;
			...
			//*/
			//let top = img.offsetTop + move_y;
			//let top = img.ctx_y + ((event.clientY - img.ctx_y) - ((event.clientY - img.ctx_y) * deltaX));//move_y;
			let top = img.ctx_y + ((cursor_offsetY - img.ctx_y) - ((cursor_offsetY - img.ctx_y) * deltaX));//move_y;
			/*
			if(top > limit_top){
				flag_offset = true;
				img.title_y -= title_h;
			...
			//*/
			//out("x = "+left+"<br>y = "+top+"<br>"+flag_offset);
			
			img.ctx_x = left | 0;
			img.ctx_y = top | 0;
			img.size = size_t * deltaY | 0;
			
			
			if(flag_offset){
				w_title_WM = size_t * res;
				img.title_x = Math.round(((ex[0] + left * res) + (width_WM / 2)) / w_title_WM);// | 0;// start_title_width //
				img.title_y = Math.round((width_WM - ((ex[3] - top * res) + width_WM / 2)) / w_title_WM);// | 0;// start_title_height //
				img.src = '';
				img.src = 'https://'+arr_s[count_i % arr_s.length]+'.tile.openstreetmap.org/'+zoom+'/'+img.title_x+'/'+img.title_y+'.png';
			...
			ctx.drawImage(img, img.ctx_x, img.ctx_y, img.size, img.size);
			//ctx.drawImage(img, img.ctx_x, img.ctx_y, size_t, size_t);
			//img.style.left = left+"px";
			//img.style.top = top+"px";
		});
		...
		// берём канвас и вставляем с новым масштабом.
		//et imagedata = ctx.getImageData();
		//ctx.fillRect(0, 0, w_m_p, h_m_p);
		//ctx.putImageData(imagedata, 0, 0, );
		/*
		// Вычисляем расстояние смещения.
		let x_offset = cursor_offsetX - cursor_offsetX * deltaY;
		let y_offset = cursor_offsetY - cursor_offsetY * deltaY;
		
		
		
		
		//ctx.drawImage(canvas, x_offset, y_offset, w_m_p * deltaY, h_m_p * deltaY);
		// обрезаем края.
		ctx.fillRect(0, 0, x_offset, h_m_p);
		ctx.fillRect(x_offset + w_m_p * deltaY, 0, w_m_p - (x_offset + w_m_p * deltaY), h_m_p);
		ctx.fillRect(x_offset, 0, w_m_p * deltaY, y_offset);
		ctx.fillRect(x_offset, y_offset + h_m_p * deltaY, w_m_p * deltaY, h_m_p - (y_offset + h_m_p * deltaY));
		//*/
	};
